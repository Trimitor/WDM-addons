...
function AtlasPOI:AddTrackingOptions()
    local menu = {
        {
            text = L["atlas_tracking_title_text"],
            isTitle = true,
        },
        {
            ...
            func = function() 
                self.db.profile.show_taxinode = not self.db.profile.show_taxinode 
            ...
            menuList = {
                {
                    ...
                    keepShownOnClick = 1,
                    checked = function() return self.db.profile.show_taxinode_opposite end,
                    func = function() 
                        self.db.profile.show_taxinode_opposite = not self.db.profile.show_taxinode_opposite
                        WorldMapFrame_Update()
                    end,
                },
            }
        },
        {
            ...
            keepShownOnClick = 1,
            checked = function() return self.db.profile.show_taxinode_continent end,
            func = function() 
                self.db.profile.show_taxinode_continent = not self.db.profile.show_taxinode_continent 
                WorldMapFrame_Update()
            end,
            hasArrow = true,
            menuList = {
                {
                    ...
                    keepShownOnClick = 1,
                    checked = function() return self.db.profile.show_taxinode_continent_opposite end,
                    func = function() 
                        self.db.profile.show_taxinode_continent_opposite = not self.db.profile.show_taxinode_continent_opposite 
                        WorldMapFrame_Update()
                    end,
                },
            },
        },
        {
            ...
            func = function() 
                self.db.profile.show_instance = not self.db.profile.show_instance 
                WorldMapFrame_Update()
            end,
        },
        {
            ...
            keepShownOnClick = 1,
            checked = function() return self.db.profile.show_zonelevel end,
            func = function() 
                self.db.profile.show_zonelevel = not self.db.profile.show_zonelevel 
                WorldMapFrame_Update()
            end,
        },
    }
    ...
    AtlasTO.Button:SetHighlightTexture("Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight")
    ...
    AtlasTO.Button.Border:SetTexture("Interface\\Minimap\\MiniMap-TrackingBorder")
...
function AtlasPOI:GetAtlasTOtext(category, opposite)
    ...
    local twidth, theight, tleft, tright, ttop, tbottom = DData:GetAtlasTextureCoords(category, faction)
    --print()
    return "|TInterface\\AddOns\\WDM\\textures\\objecticonsatlas:18:18:0:0:512:1024:"
        ..math.ceil(tleft*512)..":".. (math.ceil(tleft*512)+twidth) ..":".. math.ceil(ttop*1024)..":".. (math.ceil(ttop*1024)+theight).."|t ".. L["show_"..category.."_"..faction.."_text"]
